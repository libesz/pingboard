...
func findByID(root *etree.Element, target config.Target) (*etree.Element, error) {
	...
	if elem == nil {
		return nil, errors.New("Could not find object with ID: " + target.SvgID)
	}
	return elem, nil
}
...
func findAndGetStyleAttr(root *etree.Element, target config.Target) (*etree.Attr, bool, error) {
	...
	log.Println("Style does not exists for ID: " + string(target.SvgID) + ". Falling back to search fill property.")
	style = elem.SelectAttr("fill")
	if style != nil {
		return style, false, nil
	}
	log.Println("Fill does not exists for ID: " + string(target.SvgID) + ". Creating new attribute.")
...
func change(style *etree.Attr, embedded bool, target config.Target) {
	...
	log.Printf("SVG ID: %s, updated fill property: %s\n", target.SvgID, target.Fill)
}
